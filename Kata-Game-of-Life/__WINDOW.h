...
namespace WINDOWS_GUI {
	...
	inline WINDOW ConstructChildWindow(const std::string& type, HWND parent, HMENU id, HINSTANCE inst) {
		auto h = CreateWindow(StringToWstring(type).c_str(), L"", WS_CHILD | WS_BORDER | WS_VISIBLE, 0, 0, 0, 0, parent, id, inst, NULL);
		return WINDOW{ h };
	}

	// Create a child window
	inline WINDOW ConstructChildWindow(const std::string& type, HWND parent, unsigned long id, HINSTANCE inst) {
